...
namespace Tetris
{
    class Text{
        ...
        public Text(string text,Vector3 position,Vector3 color){
            ...
            shader = new Shader(Game.ProjectPlace +@"\TextShader.vert", Game.ProjectPlace+ @"\TextShader.frag");
        ...
        public void Render(){
            ...
            for (int i = textImages.Length-1; i >= 0; i--){
                ...
                //
                model = GameMath.TransformMatrix(new Vector3(positionx, position.Y, position.Z), Scale.X, Scale.Y, Scale.Z);
